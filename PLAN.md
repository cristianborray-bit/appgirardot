...
- [ ] **Acción pendiente para publicar el píxel:** en Vercel, pestaña "Deployments", buscar el de arriba (mensaje "chore: forzar nuevo deploy tras cambiar la rama default en GitHub") → menú "⋯" → **"Promote to Production"**. Eso debería poner esa versión (ya lista, con el píxel) en `aqualinagirardot.company` de inmediato, sin recompilar. Para que esto no se repita con cada cambio futuro, falta además encontrar dentro de Vercel (Settings → Git) el campo "Production Branch" y cambiarlo de `claude/keen-gauss-voe9u1` a `claude/youthful-hamilton-t01tfw`
- [ ] Después de publicar: confirmar en Meta Events Manager (pestaña "Probar eventos" o "Diagnóstico") que llegan los eventos `PageView` (al abrir el sitio) y `Lead` (al enviar el formulario de contacto de prueba)

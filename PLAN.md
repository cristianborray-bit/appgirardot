...
- [ ] Agregar en Vercel (Settings → Environment Variables) `NEXT_PUBLIC_META_PIXEL_ID` = `27511282028466005`, solo en el entorno "Production", y luego volver a desplegar (Deployments → ⋯ → Redeploy) para que el cambio se note
- [ ] Después de redesplegar: confirmar en Meta Events Manager (pestaña "Probar eventos" o "Diagnóstico") que llegan los eventos `PageView` (al abrir el sitio) y `Lead` (al enviar el formulario de contacto de prueba)

...
- [ ] **Publicar y confirmar el evento `IniciaChat` (acción de Cristian, desbloquea el cambio de objetivo):** el código de `IniciaChat` ya está en la rama de trabajo pero NO está vivo todavía — producción solo se actualiza con "Promote to Production" en Vercel (el campo "Production Branch" nunca se cambió, sigue en la rama vieja). Pasos: (1) "Promote to Production" en Vercel; (2) desde el celular, entrar al sitio, escribirle un mensaje al bot; (3) revisar en Events Manager (pestaña "Probar eventos" del conjunto "Aqualina Orange Girardot") que aparezca `IniciaChat`. Confirmado por datos de Meta (2026-06-20): hasta ahora el conjunto solo ha registrado `PageView` y un `Lead` de prueba; cero `IniciaChat`, coherente con que aún no está publicado
...
- [ ] **Ejecutar el cambio de objetivo (bloqueado por que `IniciaChat` esté vivo):** una vez Cristian publique `IniciaChat` y confirme que dispara, (a) yo creo la "conversión personalizada" en Meta a partir de `IniciaChat`, (b) yo creo la campaña nueva optimizando por esa conversión y la dejo EN PAUSA, (c) Cristian le da "play". No se toca la pauta (crear/activar) sin su OK explícito por paso
...
- [ ] **Volver a hacer "Promote to Production" en Vercel** — aplica al código de `IniciaChat` (2026-06-20) y también al fix del prompt de hoy (2026-06-23); ninguno de los dos está realmente en línea todavía
- [ ] **Cambiar de raíz el campo "Production Branch" en Vercel (Settings → Git)**, de `claude/keen-gauss-voe9u1` a `claude/youthful-hamilton-t01tfw` — así cada push futuro se publica solo, sin depender de que Cristian recuerde apretar "Promote to Production" cada vez (causa raíz más probable de que `IniciaChat` lleve días en cero)
- [ ] Agregar evento "chat abierto" (paso intermedio entre `PageView` e `IniciaChat`) para medir cuántas visitas del anuncio sí abren el chat — pendiente del OK de Cristian, es cambio de código nuevo
